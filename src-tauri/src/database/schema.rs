...
impl Database {
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    16 => {
                        log::info!("迁移数据库从 v16 到 v17（记录实际上游 reasoning effort）");
    ...
    /// v16 -> v17: record the named reasoning effort in the finalized upstream request.
...
mod tests {
    ...
    #[test]
    fn migrate_v16_to_v17_adds_reasoning_effort_to_request_logs() -> Result<(), AppError> {
        let conn = Connection::open_in_memory()?;
        conn.execute(
            "CREATE TABLE proxy_request_logs (request_id TEXT PRIMARY KEY)",
            [],
